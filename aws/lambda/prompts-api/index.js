 ...
 * Only allows alphanumeric, underscore, hyphen (prevents NoSQL injection)
 */
function validateTemplateId(templateId) {
  ...
  // Max length check (prevents DOS via huge strings)
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(templateId)) {
    throw new Error('Invalid template ID format. Only alphanumeric, underscore, and hyphen allowed');
  }
...
/**
 * Validate template type (whitelist approach)
 */
function validateTemplateType(templateType) {
  if (!templateType || typeof templateType !== 'string') {
    throw new Error('Template type must be specified');
  }

  const validTypes = ['theme', 'narrative', 'sfx', 'thumbnail', 'description', 'image', 'video', 'cta', 'tts'];
...
/**
 * Sanitize and validate request body for template operations
 */
function validateTemplateBody(body, operation = 'create') {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === 'GET' && pathParts.length === 0) {
      // GET / - List templates (requires ?type=theme|narrative|sfx|thumbnail|description|image|video|cta|tts)
      const templateType = queryParams.type || queryParams.template_type;
      if (!templateType) {
        return sendError(400, 'Missing required query parameter: type (theme|narrative|sfx|thumbnail|description|image|video|cta|tts)');
      }
      // SECURITY: Validate template type
      const validatedType = validateTemplateType(templateType);
      return await handleListTemplates(validatedType, queryParams);
    }
    ...
    if (method === 'GET' && pathParts.length === 2 && pathParts[0] === 'template') {
      ...
      return await handleGetTemplate(validatedType, validatedId);
    }
    ...
    if (method === 'POST' && pathParts.length === 0) {
      ...
      // SECURITY: Validate body and type
      const validatedBody = validateTemplateBody(body, 'create');
      const validatedType = validateTemplateType(validatedBody.template_type);
      return await handleCreateTemplate(validatedType, validatedBody);
    }
    ...
    if (method === 'PUT' && pathParts.length === 2 && pathParts[0] === 'template') {
      ...
      // SECURITY: Validate inputs
      const validatedBody = validateTemplateBody(body, 'update');
      const validatedType = validateTemplateType(templateType);
      const validatedId = validateTemplateId(pathParts[1]);
      return await handleUpdateTemplate(validatedType, validatedId, validatedBody);
    }
    ...
    if (method === 'DELETE' && pathParts.length === 2 && pathParts[0] === 'template') {
      ...
      const validatedType = validateTemplateType(templateType);
      const validatedId = validateTemplateId(pathParts[1]);
      return await handleDeleteTemplate(validatedType, validatedId);
    }
...
async function handleListTemplates(templateType, queryParams) {
  // templateType is already validated by validateTemplateType()
  const tableName = TABLES[templateType];
  if (!tableName) {
    return sendError(400, `Invalid template type: ${templateType}. Must be one of: theme, narrative, image, video, cta, tts`);
  }
...
async function handleGetTemplate(templateType, templateId) {
  // Both templateType and templateId are already validated
  const tableName = TABLES[templateType];
  if (!tableName) {
...
async function handleDeleteTemplate(templateType, templateId) {
  ...
  if (result.Item.is_default === true) {
    return sendError(403, 'Cannot delete default template. Default templates are protected and cannot be removed.');
  }
