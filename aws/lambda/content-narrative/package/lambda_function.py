"""
Content Narrative Lambda v4.0 - THREE PHASE STORY ENGINE

Generates content through three sequential phases:
- Phase 1a: Story Mechanics (Archetype selection + mechanics JSON)
- Phase 1b: Narrative Generation (Based on locked mechanics)
- Phase 1c: Image/Audio Prompts (With frozen character strings)

All 7 content components:
1. Narrative with SSML
2. Image Prompts for each scene (with visual subtext)
...
def log_openai_cost(channel_id, content_id, model, input_tokens, output_tokens, user_id=None):
    ...
    try:
        ...
        # Convert tokens to int (might be Decimal from DynamoDB)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Extract components from three-phase result
        story_title = three_phase_result.get('story_title', selected_topic)
        scenes = three_phase_result.get('scenes', [])
        thumbnail = three_phase_result.get('thumbnail', {})
        metadata = three_phase_result.get('metadata', {})
        mechanics = three_phase_result.get('mechanics', {})
        usage_stats = three_phase_result.get('usage', {})

        # Create mega_response format for backward compatibility with Step Functions
        mega_response = {
            'story_title': story_title,
            'scenes': scenes,
            'thumbnail': thumbnail,
            'metadata': metadata,
            'mechanics': mechanics,
            'cta_segments': [],  # TODO: Generate CTA in Phase 1b or 1c
            'description': {     # TODO: Generate description in Phase 1b
                'title': story_title,
                'description': '',
                'tags': [],
                'hashtags': []
            },
            'sfx_data': {        # SFX data from Phase 1c
                'sfx_cues': [],
                'music_track': scenes[0].get('music_track', 'dramatic_orchestral') if scenes else 'dramatic_orchestral',
                'timing_estimates': {}
            }
        }
        ...
        # Extract components for return (backward compatibility)
        ...
        print(f"\n Parsed THREE PHASE response:")
        print(f"   Title: {story_title}")
        print(f"   Scenes: {len(scenes)}")
        print(f"   Image prompts: {len([s for s in scenes if s.get('image_prompt')])}")
        print(f"   Archetype: {mechanics.get('dominant_archetype')}")
        ...
        # 8. Log cost (three phases)
        timestamp = datetime.utcnow().isoformat() + 'Z'
        narrative_id = timestamp.replace(':', '').replace('-', '').replace('.', '')[:20]

        total_tokens = int(usage_stats.get('total_tokens', 0))
        # Estimate input/output split (rough 60/40)
        ...
        # Add mechanics to DynamoDB storage
        mechanics_decimal = convert_floats_to_decimal(mechanics)

        content_table.put_item(
            Item={
                'channel_id': channel_id,
                'created_at': timestamp,
                'type': 'three_phase_generation',
                'topic': selected_topic,
                'story_title': story_title,
                ...
                'usage_stats': convert_floats_to_decimal(usage_stats)
            }
        )
