    ...
    try:
        ...
        if topic_id and series_id and episode_number and generate_episode_summary and update_topic_with_summary:
            ...
            try:
                ...
                # Sprint 4: AUTO_UPDATE SeriesState after episode generation
                print(f" Sprint 4: AUTO_UPDATE SeriesState for series {series_id}, episode {episode_number}")
                try:
                    series_update_response = lambda_client.invoke(
                    ...
                    series_result = json.loads(series_update_response['Payload'].read())
                    if series_result.get('statusCode') == 200:
                        body = json.loads(series_result['body'])
                        if body.get('success'):
                            print(f" SeriesState updated: episodes_generated={body.get('data', {}).get('episodes_generated', 'N/A')}")
                        else:
                            print(f" WARNING: SeriesState update returned error: {body.get('error')}")
                    else:
                        print(f" WARNING: SeriesState update failed with status {series_result.get('statusCode')}")

                except Exception as e:
                    print(f" ERROR updating SeriesState: {e}")
                    import traceback
                    traceback.print_exc()
