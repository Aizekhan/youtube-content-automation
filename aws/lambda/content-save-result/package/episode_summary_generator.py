...
{{
  ...
  "characters_introduced": [
    {{
      "name": "Character Name",
      "description": "Brief visual and personality description",
      "gender": "male|female|neutral",
      "age_group": "child|young_adult|adult|elder",
      "character_type": "human|animal|spirit|creature"
    }}
  ],
  "open_threads": [
...
2. characters_introduced: For each character, determine gender from context. For animals/creatures, use the gender implied by narrative or personality (e.g., wise old owl = male/elder, playful fox girl = female/young_adult)
3. open_threads: Only include threads with UNRESOLVED tension (not background info)
4. closed_threads: Only include threads that were FULLY resolved in this episode
5. Keep it concise - this will be context for future episodes
...
Generate the JSON now:"""
    ...
    try:
        ...
        # Normalize characters_introduced to new format (support both old string and new dict format)
        normalized_characters = []
        for char in summary.get('characters_introduced', []):
            if isinstance(char, dict):
                # New format - already has gender, age_group, character_type
                ...
                normalized_characters.append({
                    ...
                    'gender': 'neutral',  # Default - will be inferred by voice mapper
                    'age_group': 'adult',
                    'character_type': 'human'
                })

        summary['characters_introduced'] = normalized_characters

        print(f"Generated episode summary: {len(summary['episode_summary'])} chars, {len(summary['characters_introduced'])} characters, {len(summary['open_threads'])} open threads")
